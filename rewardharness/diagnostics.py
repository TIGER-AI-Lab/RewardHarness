...
def check_imports() -> bool:
    ...
    if missing:
        print(
            f"  [{FAIL}] missing modules: {', '.join(missing)} — `pip install -r requirements.txt`"
        )
        return False
...
def check_endpoints(endpoints_path: str, timeout: float = 3.0) -> bool:
    ...
    if served_mismatch:
        print(
            f"  [{WARN}] {len(served_mismatch)} endpoint(s) serve a different model id than REWARDHARNESS_SUBAGENT_MODEL — see TROUBLESHOOTING.md (404 mismatch)."
        )
