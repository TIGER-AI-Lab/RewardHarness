...
_ENTRY_NAME_RE = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")
...
class Library:
    def __init__(self, base_dir: str):
        ...
        self.metadata = {"id": os.path.basename(os.path.abspath(base_dir)), "status": "active"}
        self.load_registry()
    ...
    def load_registry(self):
        ...
        if os.path.exists(self.registry_path):
            ...
            if document.get("schema_version") == REGISTRY_SCHEMA_VERSION:
                self.metadata = dict(document.get("library", self.metadata))
    ...
    def registry_document(self, registry: dict | None = None) -> dict:
        """Serialize the internal flat registry as the public v2 document."""
        entries = {}
        for name, info in (registry or self.registry).items():
            serialized = dict(info)
            serialized["kind"] = serialized.pop("type")
            entries[name] = serialized
        return {
    ...
    def _validate_name(name: str) -> None:
        if not _ENTRY_NAME_RE.fullmatch(name):
            raise ValueError(
                "Library entry names must be lowercase kebab-case without path separators"
            )
    ...
    def save_registry(self, merge: bool = True):
        ...
        with FileLock(lock_path):
            if merge:
                # Re-read the on-disk registry so concurrent additions survive.
                if os.path.exists(self.registry_path):
                    with open(self.registry_path) as f:
                        document = json.load(f)
                    if document.get("schema_version") == REGISTRY_SCHEMA_VERSION:
                        disk_registry = {
                            name: {**info, "type": info.get("kind", info.get("type"))}
                            for name, info in document.get("entries", {}).items()
                        }
                    else:
                        disk_registry = document
                else:
                    disk_registry = {}
                disk_registry.update(self.registry)
                self.registry = disk_registry

            fd, tmp_path = tempfile.mkstemp(dir=os.path.dirname(self.registry_path), suffix=".tmp")
            try:
                with os.fdopen(fd, "w") as f:
                    json.dump(self.registry_document(), f, indent=2)
                    f.write("\n")
    ...
    def update_skill(self, name: str, new_content_md: str, new_description: str | None = None):
    ...
    def snapshot(self) -> dict:
        ...
        """
        files = {}
        for _name, info in self.registry.items():
            rel_path = info["path"]
