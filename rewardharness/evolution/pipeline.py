...
class SelfEvolutionPipeline:
    def __init__(
        self,
        config: dict | RewardHarnessConfig,
        ...
        typed_config = (
            config
            if isinstance(config, RewardHarnessConfig)
            else RewardHarnessConfig.from_mapping(config)
        )
        config = typed_config.to_legacy_dict()

        self.config = config
        self.lib_dir = library_dir or str(default_library_path())
        self.library = Library(self.lib_dir)
        self.router = Router(self.library, model=config["gemini"]["model"])

        endpoints_file = str(default_endpoints_path())
        ...
        self.results_dir = results_dir or str(PROJECT_ROOT / "results")
    ...
    def _prune_library(self, val_split: list) -> dict:
        ...
        all_entries = [(s["name"], "skill") for s in summaries["skills"]] + [
        ...
        for name, entry_type in all_entries:
            ...
            try:
                ...
                logger.info(
                    f"Prune ablation: removing '{name}' ({entry_type}) -> "
        ...
        logger.info(
            f"Pruning complete: removed {len(pruned_skills)} skills, {len(pruned_tools)} tools"
        )
    ...
    def run_iteration(self, examples: list) -> list:
        ...
        with ThreadPoolExecutor(max_workers=min(len(examples), 8)) as pool:
            skill_contexts = list(
                pool.map(lambda ex: self.router.prepare_context(ex["prompt"]), examples)
            )

        # 2. Batch evaluate
        predictions = self.sub_agent.batch_evaluate(
            examples, skill_contexts, max_workers=self.max_workers
        )

        # 3. Compare with GT
        results = []
        for ex, pred in zip(examples, predictions, strict=False):
    ...
    def _checkpoint(
        ...
        val_acc_after_skills: float | None = None,
        val_acc_after_tools: float | None = None,
    ):
        """Save checkpoint for an iteration."""
        iter_dir = os.path.join(self.checkpoint_dir, f"iter_{iteration}")
        os.makedirs(iter_dir, exist_ok=True)

        # Save versioned registry
        with open(os.path.join(iter_dir, "registry.json"), "w") as f:
            json.dump(self.library.registry_document(snap["registry"]), f, indent=2)
            f.write("\n")

        # Save all versioned Skill/Tool documents.
        for relative_path, content in snap.get("files", {}).items():
            destination = os.path.join(iter_dir, relative_path)
            os.makedirs(os.path.dirname(destination), exist_ok=True)
            with open(destination, "w") as f:
                f.write(content)

        # Always save best_val_acc so resume loads the correct baseline.
        metadata = {
            "iteration": iteration,
            "schema_version": 2,
            ...
            "n_skills": sum(info["type"] == "skill" for info in snap.get("registry", {}).values()),
            "n_tools": sum(info["type"] == "tool" for info in snap.get("registry", {}).values()),
        }
        if signals:
    ...
    def _load_checkpoint(self, iter_dir: str) -> tuple:
        ...
        with open(os.path.join(iter_dir, "registry.json")) as f:
            registry_document = json.load(f)
        if registry_document.get("schema_version") == 2:
            registry = {
                name: {**info, "type": info.get("kind", info.get("type"))}
                for name, info in registry_document.get("entries", {}).items()
            }
        else:
            registry = registry_document

        snap: dict[str, Any] = {"registry": registry, "files": {}}
            ...
            for name in os.listdir(skills_dir):
                ...
                if os.path.exists(skill_file):
                    with open(skill_file) as f:
                        snap["files"][f"skills/{name}/SKILL.md"] = f.read()
            ...
            for name in os.listdir(tools_dir):
                ...
                if os.path.exists(tool_file):
                    with open(tool_file) as f:
                        snap["files"][f"tools/{name}/SKILL.md"] = f.read()

        return metadata, snap

    def get_latest_checkpoint(self) -> tuple[int | None, str | None]:
    ...
    def evolve(self, n_iterations: int = 5, train_split=None, val_split=None, resume: bool = False):
        ...
        for iteration in range(start_iter, n_iterations):
            ...
            if iteration == 0:
                ...
                logger.info(
                    f"Signals: {len(skill_updates)} skill updates, {len(tool_updates)} tool updates"
                )
                ...
                if skill_updates:
                    ...
                    if val_acc_after_skills >= prev_val_acc - explore_margin:
                        ...
                        logger.info(
                            f"Phase A KEEP: val_acc {old_val_acc:.4f} -> {val_acc_after_skills:.4f}"
                        )
                    else:
                        skill_action = "rollback"
                        self.evolver.restore(snap_before_skills)
                        logger.info(
                            f"Phase A ROLLBACK: val_acc {val_acc_after_skills:.4f} < prev {prev_val_acc:.4f} - {explore_margin}"
                        )
                ...
                if tool_updates:
                    ...
                    if val_acc_after_tools >= prev_val_acc - explore_margin:
                        ...
                        logger.info(
                            f"Phase B ROLLBACK: val_acc {val_acc_after_tools:.4f} < prev {prev_val_acc:.4f} - {explore_margin}"
                        )
                ...
                log_entry = {
                    ...
                    if val_acc_after_tools is not None
                    else (
