...
class ChainAnalyzer:
    def __init__(self, model: str = "gemini-3.1-pro-preview", complete=None):
        self.model = model
    ...
    def analyze(self, examples: list, current_library: dict) -> dict:
        ...
        prompt = ANALYSIS_PROMPT.format(
            skills_summary=skills_summary, tools_summary=tools_summary, examples_text=examples_text
        )
        ...
        for attempt in range(4):
            try:
                ...
                if "429" in err_str and attempt < 3:
                    wait = (attempt + 1) * 15
                    logger.warning(
                        "ChainAnalyzer 429 rate limit, retrying in %ds (attempt %d/4)",
                        wait,
                        attempt + 1,
                    )
                    time.sleep(wait)
        ...
        if response_text is None:
            return {
                "skill_updates": [],
                "tool_updates": [],
                "analysis_summary": "All retries exhausted",
            }
        ...
        try:
            ...
            logger.error("Failed to parse chain analysis response as JSON: %s", response_text[:300])
            signals = {
                "skill_updates": [],
                "tool_updates": [],
