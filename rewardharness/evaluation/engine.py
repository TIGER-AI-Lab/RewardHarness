...
from rewardharness.paths import score_guidelines_path
...
class SubAgent:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    wait = 2**attempt
                    logger.warning(
                        "vLLM call failed (attempt %d): %s, retrying in %ds", attempt + 1, e, wait
                    )
                    time.sleep(wait)
                else:
                    logger.error("vLLM call failed after %d attempts: %s", self.max_retries, e)
                    raise
        raise RuntimeError("vLLM retry loop exited unexpectedly")
    ...
    def evaluate(
        self, source_img: str, edited_A: str, edited_B: str, prompt: str, skill_context: str
    ) -> dict:
    ...
    def batch_evaluate(self, examples: list, skill_contexts: list, max_workers: int = 128) -> list:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_idx = {executor.submit(_eval, i): i for i in range(len(examples))}
            for future in as_completed(future_to_idx):
                idx = future_to_idx[future]
                try:
                    results[idx] = future.result()
                except Exception as e:
                    logger.error("batch_evaluate error for example %d: %s", idx, e, exc_info=True)
                    fallback = dict(FALLBACK_ANSWER)
                    fallback["chain"] = f"error: {e}"
                    results[idx] = fallback

        return [result if result is not None else dict(FALLBACK_ANSWER) for result in results]
