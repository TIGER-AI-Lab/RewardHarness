...
class Router:
    def __init__(self, library, model: str = "gemini-3.1-pro-preview", complete=None):
        self.lib = library
        self.model = model
    ...
    def prepare_context(self, prompt: str) -> str:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    ...
                    continue
                logger.warning(
                    "Router Gemini API unavailable, using all library entries: %s", str(e)[:100]
                )
                selected = {
        ...
        logger.debug(
            "Router selected skills=%s tools=%s (attempts=%d) for prompt='%s'",
            selected.get("skills", []),
            selected.get("tools", []),
            attempt + 1,
            prompt[:80],
    ...
    def _parse_response(response_text: str):
        ...
        if not isinstance(skills, list) or not isinstance(tools, list):
            return None
        if not all(isinstance(name, str) for name in [*skills, *tools]):
            return None
        return {"skills": list(dict.fromkeys(skills)), "tools": list(dict.fromkeys(tools))}
